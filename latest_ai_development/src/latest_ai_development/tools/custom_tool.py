...
class WebRequestTool(BaseTool):
    ...
    description: str = (
        "This is a tool for sending request to a website. It return the content of the website."
    )
...
class WriteToFileTool(BaseTool):
    ...
    description: str = "This is a tool for writing given content to file"
    args_schema: Type[BaseModel] = WriteToFileToolInput

    def _run(self, file_name: str, content: str) -> str:
        with open(file_name, "wb") as f:
            res = bytes(content, "utf-8")
            out = f.write(bytearray(res))
            if out == 0:
